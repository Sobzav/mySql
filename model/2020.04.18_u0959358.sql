...
-- -----------------------------------------------------
-- Table `u0959358_sobzav`.`package_nature`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `u0959358_sobzav`.`package_nature` ;

CREATE TABLE IF NOT EXISTS `u0959358_sobzav`.`package_nature` (
  `id` INT UNSIGNED NOT NULL,
  `name` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`id`))
ENGINE = InnoDB
...
CREATE TABLE IF NOT EXISTS `u0959358_sobzav`.`package` (
  ...
  INDEX `fk_package_to_photo_idx` (`nature_id` ASC),
  CONSTRAINT `fk_package_to_package_nature`
    FOREIGN KEY (`nature_id`)
    REFERENCES `u0959358_sobzav`.`package_nature` (`id`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT)
ENGINE = InnoDB
...
-- -----------------------------------------------------
-- Table `u0959358_sobzav`.`place_nature`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `u0959358_sobzav`.`place_nature` ;

CREATE TABLE IF NOT EXISTS `u0959358_sobzav`.`place_nature` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`id`))
ENGINE = InnoDB
...
-- -----------------------------------------------------
-- Table `u0959358_sobzav`.`place_pattern`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `u0959358_sobzav`.`place_pattern` ;

CREATE TABLE IF NOT EXISTS `u0959358_sobzav`.`place_pattern` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(45) NOT NULL COMMENT 'Наименование упаковки\nКак пример:\n	- Пакет полиэтиленовый\n	- Ячейка пластикового контейнера\n	- Контейнер пластиковый\n	- Коробка картонная\n	- Ящик деревянный\n	- Контейнер металический\n	- Палета деревянная\n	- Площадка открытая для хранения Крупногабаритного товара\n	и т.д….',
  `code` CHAR(7) NOT NULL COMMENT 'Обозначение\nУникальное сочетание букв и цифр для удобной и однозначной идентификации упаковки\nКак пример:\n	B001 - полиэтиленовый пакет №1 (размер h=2мм х w=50мм х l=100мм)\n	B002 - пластиковая ячейка контейнера (всего 25 ячеек 5 х 5, размер ячейки h=25мм х w=50мм х l=50мм)\n	и т.д….',
  `iwx` MEDIUMINT UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Внутренний размер - Ширина  элемента в положении режима эксплуатации в мм.',
  `iwy` MEDIUMINT UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Внутренний размер - Высота  элемента в положении режима эксплуатации в мм.',
  `iwz` MEDIUMINT UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Внутренний размер - Глубина  элемента в положении режима эксплуатации в мм.',
  `volume` DOUBLE GENERATED ALWAYS AS ((iwx * iwy * iwz)) STORED COMMENT 'Столбец в контейнере котором расположен данный тип места',
  `payload` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Грузоподъемность упаковки в граммах.',
  `nature_id` INT UNSIGNED NULL COMMENT 'Id природы места',
  `wx` MEDIUMINT UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Внешний размер - Ширина  элемента в положении режима эксплуатации в мм.',
  `wy` MEDIUMINT UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Внешний размер - Высота  элемента в положении реж - Высота  элемента в положении режима эксплуатации - Ширина  элемента в положении режима эксплуатацииима эксплуатации в мм.',
  `wz` MEDIUMINT UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Внешний размер - Глубина  элемента в положении режима эксплуатации в мм.',
  `archetype_id` INT UNSIGNED NULL COMMENT 'Id базового типа из этой же таблицы, на основе которого был создан не базовый тип',
  `bycoordinates` TINYINT UNSIGNED NULL DEFAULT 0,
  `color` CHAR(6) NOT NULL COMMENT 'Цвет упаковки на схеме в графическом интерфейсе',
  `depth` TINYINT UNSIGNED NULL DEFAULT 2 COMMENT '1 - элемент отобразит себя\\n2 - элемент отобразит себя и внутренние элементы\\n3 - элемент отобразит 2 уровня внутренних элементов',
  `view` TINYINT UNSIGNED NULL DEFAULT 0 COMMENT '0 - элемент будет отображен как есть\\\\\\\\n1 - элемент будет отображен повернутым по оси x',
  `created` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated` TIMESTAMP NULL ON UPDATE CURRENT_TIMESTAMP,
  `deleted` TIMESTAMP NULL,
  `sub1_id` INT UNSIGNED NULL,
  `inrow1` TINYINT UNSIGNED NULL DEFAULT 0,
  `position1` TINYINT UNSIGNED NULL DEFAULT 0 COMMENT '0 - элемент будет отображен как есть\\\\\\\\n1 - элемент будет отображен повернутым по оси y',
  `x1` INT UNSIGNED NULL COMMENT 'Ряд в контейнере котором расположен данный тип места',
  `y1` INT UNSIGNED NULL COMMENT 'Столбец в контейнере котором расположен данный тип места',
  `sub2_id` INT UNSIGNED NULL,
  `inrow2` TINYINT UNSIGNED NULL DEFAULT 0,
  `position2` TINYINT UNSIGNED NULL DEFAULT 0 COMMENT '0 - элемент будет отображен как есть\\\\\\\\n1 - элемент будет отображен повернутым по оси y',
  `x2` INT UNSIGNED NULL COMMENT 'Ряд в контейнере котором расположен данный тип места',
  `y2` INT UNSIGNED NULL COMMENT 'Столбец в контейнере котором расположен данный тип места',
  `sub3_id` INT UNSIGNED NULL,
  `inrow3` TINYINT UNSIGNED NULL DEFAULT 0,
  `position3` TINYINT UNSIGNED NULL DEFAULT 0 COMMENT '0 - элемент будет отображен как есть\\\\\\\\n1 - элемент будет отображен повернутым по оси y',
  `x3` INT UNSIGNED NULL COMMENT 'Ряд в контейнере котором расположен данный тип места',
  `y3` INT UNSIGNED NULL COMMENT 'Столбец в контейнере котором расположен данный тип места',
  `sub4_id` INT UNSIGNED NULL,
  `inrow4` TINYINT UNSIGNED NULL DEFAULT 0,
  `position4` TINYINT UNSIGNED NULL DEFAULT 0 COMMENT '0 - элемент будет отображен как есть\\\\\\\\n1 - элемент будет отображен повернутым по оси y',
  `x4` INT UNSIGNED NULL COMMENT 'Ряд в контейнере котором расположен данный тип места',
  `y4` INT UNSIGNED NULL COMMENT 'Столбец в контейнере котором расположен данный тип места',
  `sub5_id` INT UNSIGNED NULL,
  `inrow5` TINYINT UNSIGNED NULL DEFAULT 0,
  `position5` TINYINT UNSIGNED NULL DEFAULT 0 COMMENT '0 - элемент будет отображен как есть\\\\\\\\n1 - элемент будет отображен повернутым по оси y',
  `x5` INT UNSIGNED NULL COMMENT 'Ряд в контейнере котором расположен данный тип места',
  `y5` INT UNSIGNED NULL COMMENT 'Столбец в контейнере котором расположен данный тип места',
  `sub6_id` INT UNSIGNED NULL,
  `inrow6` TINYINT UNSIGNED NULL DEFAULT 0,
  `position6` TINYINT UNSIGNED NULL DEFAULT 0 COMMENT '0 - элемент будет отображен как есть\\\\\\\\n1 - элемент будет отображен повернутым по оси y',
  `x6` INT UNSIGNED NULL COMMENT 'Ряд в контейнере котором расположен данный тип места',
  `y6` INT UNSIGNED NULL COMMENT 'Столбец в контейнере котором расположен данный тип места',
  `sub7_id` INT UNSIGNED NULL,
  `inrow7` TINYINT UNSIGNED NULL DEFAULT 0,
  `position7` TINYINT UNSIGNED NULL DEFAULT 0 COMMENT '0 - элемент будет отображен как есть\\\\\\\\n1 - элемент будет отображен повернутым по оси y',
  `x7` INT UNSIGNED NULL COMMENT 'Ряд в контейнере котором расположен данный тип места',
  `y7` INT UNSIGNED NULL COMMENT 'Столбец в контейнере котором расположен данный тип места',
  `sub8_id` INT UNSIGNED NULL,
  `inrow8` TINYINT UNSIGNED NULL DEFAULT 0,
  `position8` TINYINT UNSIGNED NULL DEFAULT 0 COMMENT '0 - элемент будет отображен как есть\\\\\\\\n1 - элемент будет отображен повернутым по оси y',
  `x8` INT UNSIGNED NULL COMMENT 'Ряд в контейнере котором расположен данный тип места',
  `y8` INT UNSIGNED NULL COMMENT 'Столбец в контейнере котором расположен данный тип места',
  `sub9_id` INT UNSIGNED NULL,
  `inrow9` TINYINT UNSIGNED NULL DEFAULT 0,
  `position9` TINYINT UNSIGNED NULL DEFAULT 0 COMMENT '0 - элемент будет отображен как есть\\\\\\\\n1 - элемент будет отображен повернутым по оси y',
  `x9` INT UNSIGNED NULL COMMENT 'Ряд в контейнере котором расположен данный тип места',
  `y9` INT UNSIGNED NULL COMMENT 'Столбец в контейнере котором расположен данный тип места',
  UNIQUE INDEX `code_UNIQUE` (`code` ASC),
  INDEX `fk_place_pattern_to_place_pattern_idx` (`archetype_id` ASC),
  INDEX `fk_place_pattern_to_place_nature_idx` (`nature_id` ASC),
  PRIMARY KEY (`id`),
  INDEX `fk_place_pattern_sub1_to_place_pattern_idx` (`sub1_id` ASC),
  INDEX `fk_place_pattern_sub2_to_place_pattern_idx` (`sub2_id` ASC),
  INDEX `fk_place_pattern_sub3_to_place_pattern_idx` (`sub3_id` ASC),
  INDEX `fk_place_pattern_sub4_to_place_pattern_idx` (`sub4_id` ASC),
  INDEX `fk_place_pattern_sub5_to_place_pattern_idx` (`sub5_id` ASC),
  INDEX `fk_place_pattern_sub6_to_place_pattern_idx` (`sub6_id` ASC),
  INDEX `fk_place_pattern_sub7_to_place_pattern_idx` (`sub7_id` ASC),
  INDEX `fk_place_pattern_sub8_to_place_pattern_idx` (`sub8_id` ASC),
  INDEX `fk_place_pattern_sub9_to_place_pattern_idx` (`sub9_id` ASC),
  CONSTRAINT `fk_place_to_place_nature`
    FOREIGN KEY (`nature_id`)
    REFERENCES `u0959358_sobzav`.`place_nature` (`id`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT,
  CONSTRAINT `fk_place_pattern_to_place_pattern`
    FOREIGN KEY (`archetype_id`)
    REFERENCES `u0959358_sobzav`.`place_pattern` (`id`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT,
...
-- -----------------------------------------------------
-- Table `u0959358_sobzav`.`stock_unit`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `u0959358_sobzav`.`stock_unit` ;

CREATE TABLE IF NOT EXISTS `u0959358_sobzav`.`stock_unit` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(45) NOT NULL COMMENT 'Наименование упаковки\nКак пример:\n	- Пакет полиэтиленовый\n	- Ячейка пластикового контейнера\n	- Контейнер пластиковый\n	- Коробка картонная\n	- Ящик деревянный\n	- Контейнер металический\n	- Палета деревянная\n	- Площадка открытая для хранения Крупногабаритного товара\n	и т.д….',
  `address` CHAR(16) NOT NULL COMMENT 'Обозначение\nУникальное сочетание букв и цифр для удобной и однозначной идентификации упаковки\nКак пример:\n	B001 - полиэтиленовый пакет №1 (размер h=2мм х w=50мм х l=100мм)\n	B002 - пластиковая ячейка контейнера (всего 25 ячеек 5 х 5, размер ячейки h=25мм х w=50мм х l=50мм)\n	и т.д….',
  `place_pattern_id` INT UNSIGNED NULL COMMENT 'Id базового типа из этой же таблицы, на основе которого был создан не базовый тип',
  `created` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated` TIMESTAMP NULL ON UPDATE CURRENT_TIMESTAMP,
  `deleted` TIMESTAMP NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_stock_unit_to_place_pattern_idx` (`place_pattern_id` ASC),
  UNIQUE INDEX `full_addres_UNIQUE` (`address` ASC),
  CONSTRAINT `fk_stock_unit_to_place_pattern`
    FOREIGN KEY (`place_pattern_id`)
    REFERENCES `u0959358_sobzav`.`place_pattern` (`id`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT)
ENGINE = InnoDB
